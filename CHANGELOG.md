...
### Fixed
- **`CHANGELOG.md` is now published with the package.** `files` listed only `dist` and `src`, and npm's automatic inclusions cover `README.md` and `LICENSE` but not the changelog, so an installed copy had no record of what changed between versions. Anyone reviewing an upgrade from `node_modules` had to go to GitHub to find it.
